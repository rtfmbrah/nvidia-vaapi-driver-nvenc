...
Keep only wrap definitions in git:
- `subprojects/ff-nvcodec-headers.wrap`

Do not commit generated Meson subproject artifacts:
- `subprojects/.wraplock`
- `subprojects/packagecache/`
- unpacked fallback source directories such as `subprojects/nv-codec-headers-*`

`Video_Codec_Interface` / `Video_Codec_SDK` checkouts are optional for this fork and are not required for normal builds when `ffnvcodec` is available.

## Maintenance files

- `.github/`: recommended to keep. It gives you reproducible CI build/sanity checks for pull requests.
- `headers.in` and `extract_headers.sh`: keep as maintainer tooling for refreshing `nvidia-include/` from NVIDIA open-gpu-kernel-modules snapshots.
- `meson_options.txt`: removed because it was empty and unused.

## Development workflow suggestion

- `main` (or `master`): stable baseline
- `feature/*`: implementation work
- `opt/*`: optimization/robustness work
- `debug/*`: temporary instrumentation branches

Recommended sequence:
1. stabilize behavior
2. remove temporary debug paths
3. optimize (copy reduction/zero-copy experiments)
